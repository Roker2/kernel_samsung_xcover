...
static unsigned char gv_call_mode = FALSE;
static unsigned char gv_need_call_compensation = FALSE;
...
int spa_Lfs_Access(int rw_op, spa_poweroff_t value)
{
	...
	// S5690_CHN Reset because this code  -start-
	/*
	size = filp->f_op->read(filp,  a_rcvBuf, SPA_POWEROFF_BUF_SIZE-1, &filp->f_pos);
	printk(KERN_INFO "Size = %d\n", size);
	printk(KERN_ALERT "%s\n", a_rcvBuf);
	*/
	//S5690_CHN Reset because this code  -end-
...
static void spa_Bat_AverageVolt(unsigned char v_update)
{
	...
	if(v_update == TRUE) {
		for(i = 0; i < BAT_VOLTAGE_SAMPLE; i++) {
			...
			if(gv_need_call_compensation == TRUE)
				a_VbatVoltValues[i] = v_vbat_adc + COMPENSATION_PHONE_CALL;
			else
	...
	printk(KERN_ALERT "%s: AverageVolt = <%ld>\n", __func__, gv_AverageVbat);
}
/*  - Get Voltage value */
...
static unsigned int spa_Get_Capacity(void) 
{
	...
	SPA_DBG_MSG(2,  "%s : Prev Capacity = <%d>\n", __func__, v_prevCapVal); 
		...
		if( SPA_GET_STATUS(STATUS_FULL) || SPA_GET_STATUS(STATUS_IS_CHARGING) || SPA_GET_STATUS(STATUS_RECHG) ) {
			 if(v_prevCapVal < v_capacity)  {
				#ifdef FEAT_EN_FG_PLUSONE
				if(gv_FgOneUpdate &&(v_capacity <= 100)) {
					if(v_prevCapVal < 100)
			...
			if(v_capacity  < v_prevCapVal) {
				...
				*/
				if(gv_IsSuspendState)
					...
					if(v_capacity+1  < v_prevCapVal) {
						v_capacity = v_prevCapVal - 2;
					}
					else {	
					        v_capacity = v_prevCapVal -1;
					}
...
static void spa_bat_work(struct work_struct *work)
{
	...
	schedule_delayed_work(&pdata->battery_work, HZ*BAT_WORK_INTERVAL);
 	...
 	if(gv_call_mode == FALSE)
		gv_need_call_compensation = FALSE;
...
{
	unsigned int v_capacity;
    unsigned int call_compensation = 0;

    if(gv_call_mode == TRUE) {
        call_compensation = COMPENSATION_PHONE_CALL;
    }
	...
	if ((SPA_GET_STATUS(STATUS_AC) == DISCONNECT) \
		...
		if(v_capacity > BATT_FG_15PCTG) {
			spa_SetVbatThresholds(BATT_FG_15P_V - call_compensation, 0);
			gv_NextLevelAlarmValues = BATT_FG_15PCTG;
		}
		else if(v_capacity > BATT_FG_05PCTG) {
			spa_SetVbatThresholds(BATT_FG_05P_V - call_compensation, 0);
			gv_NextLevelAlarmValues = BATT_FG_05PCTG;
		}
		else {
			spa_SetVbatThresholds(BATT_LOW_VOLT - call_compensation, 0);
...
void spa_voltThresholdEvent(int event_status, void *data)
{
	...
	if ((SPA_GET_STATUS(STATUS_AC) == DISCONNECT) \
		...
		if(gv_NextLevelAlarmValues + 1 >= calculate_batt_level(gv_AverageVbat)) {
			...
			printk(KERN_ALERT "%s : report %d%% capacity to Android platform\n", __func__, gv_NextLevelAlarmValues);
		}
	}
	#endif
...
static int spa_Suspend(struct i2c_client *client, pm_message_t state)
{
	...
	if(gv_call_mode == TRUE) {
		gv_need_call_compensation = TRUE;
		call_compensation = COMPENSATION_PHONE_CALL;
	}
...
static int spa_ioctl(struct inode *inode,
 					 struct file *file,
 					 unsigned int cmd,
 					 unsigned long arg)
{
	...
	switch(cmd) {
		case SPA_CALL_START:
			gv_call_mode = TRUE;	
			SPA_DBG_MSG(1, "gv_call_mode is TRUE\n");
			break;
		case SPA_CALL_FINISH:
			gv_call_mode = FALSE;
			SPA_DBG_MSG(1, "gv_call_mode is FALSE\n");
			break;
		default:
			return -ENOTTY;
	}	
...
static const struct file_operations spa_fops = {
 	.owner		= THIS_MODULE,
 	.ioctl		= spa_ioctl,
};
 
 static struct miscdevice spa_miscdev = {
