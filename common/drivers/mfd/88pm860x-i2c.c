...
void pm860x_audio_shutdown(void)
{
	/* AUDIO path must be closed gracefully before reset
	* Uses for "check audio status" the cache/shadow
	* (and so implemented here but not in any other place).
	*/
	u8 data = 0;
#if defined(CACHE_88PM860X_I2C)
	if (pm8607_cache[0xe1].data)
		pm860x_write_device(pm8607_i2c_client, 0xe1, 1, &data);
	if (pm8607_cache[0x42].data && (pm8607_cache[0x42].data != 4))
		pm860x_write_device(pm8607_i2c_client, 0x42, 1, &data);
#else
	pm860x_write_device(pm8607_i2c_client, 0xe1, 1, &data);
	pm860x_write_device(pm8607_i2c_client, 0x42, 1, &data);
#endif
}
EXPORT_SYMBOL(pm860x_audio_shutdown);
