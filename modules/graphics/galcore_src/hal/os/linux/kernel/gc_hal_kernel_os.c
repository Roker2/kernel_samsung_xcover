	...
	)
{
    ...
    if (Logical)
    {
		...
		if (mdlMap == gcvNULL || mdlMap->vmaAddr == gcvNULL)
		{
		 ...
		 _UnmapUserLogical(current->tgid, mdlMap->vmaAddr, mdl->numPages * PAGE_SIZE);
...
gceSTATUS gckOS_FreeProcessResource(IN gckOS Os, gctUINT32 pid)
{
    ...
    while(mdl)
    {
        ...
        if(mdl->pid == pid)
        {
			...
			{
				gcmkVERIFY_OK(
					gckOS_UnmapUserLogical(Os, (gctPHYS_ADDR) mdl, mdl->numPages * PAGE_SIZE, mdlMap->vmaAddr));
			}

			gcmkVERIFY_OK(
				gckOS_FreeNonPagedMemory(Os, mdl->numPages * PAGE_SIZE, (gctPHYS_ADDR) mdl, (gctPOINTER) mdl));
        }
