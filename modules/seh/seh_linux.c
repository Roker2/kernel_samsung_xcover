...
// SLR fix
static int bStressTest;
static int bStartRecovery;
// SLR fix
...
void seh_int_handler_high(struct work_struct *data)
{
	...
	if (seh_dev)
	{
		...
		seh_mipsram_mark_end_of_log();
// SLR fix
		ERRMSG("Receive SEH_IOCTL_COM_ASSERT\n");

		/* Config PMIC WatchDog to guaranty reset in 8sec
		* bits 6,[5:2]: 0x5A=8s, 0x5E=16s, 0x32s, 0x62=32s
		*/
		pm860x_codec_reg_write(0x0B, 0x5A);

		//pm860x_codec_reg_set_bits(0xAE, 1<<7, 1<<7); - don't
// SLR fix
...
// SLR fix
    ...
    case _EehInsertComm2Reset:
    {
		...
		if (copy_from_user(&resetParams, params.params,sizeof(EehInsertComm2ResetParam)))
			return -EFAULT;
// SLR fix
    	ERRMSG("EehInsertComm2Reset (Silent) asserttype: %d\n", resetParams.AssertType);
		/* Recovery STARTED
		* Change PMIC WatchDog from Assert's short value to long 32sec
		*/
		pm860x_codec_reg_write(0x0B, 0x62);
// SLR fix
...
// SLR fix			
			...
			{	/* Stop the PMIC-WDT */
				bStartRecovery = 0;
				pm860x_codec_reg_write(0x0B, 0x42);
			}
// SLR fix			
			if (down_interruptible(&seh_dev->read_sem))
				return -EFAULT;
		...
		case SEH_IOCTL_APP_ASSERT:
		{
			...
			pm860x_codec_reg_write(0x0B, 0x5A);
// SLR fix
			ERRMSG("Receive SEH_IOCTL_APP_ASSERT\n");
			SEH_PM_WAKE_LOCK();
// SLR fix			
