...
#define DEF_IOCTL_BUF_SIZE    2048
    buf_size = size + sizeof(wait_queue) + sizeof(mlan_ioctl_req);
    if (buf_size < DEF_IOCTL_BUF_SIZE)
        buf_size = DEF_IOCTL_BUF_SIZE;
    if (!(req = (mlan_ioctl_req *) kmalloc(buf_size, GFP_ATOMIC))) {
        PRINTM(MERROR, "%s: Fail to alloc ioctl buffer\n", __FUNCTION__);
        LEAVE();
        return NULL;
    }
    memset((t_u8 *) req, 0, buf_size);
    req->pbuf = (t_u8 *) req + sizeof(mlan_ioctl_req) + sizeof(wait_queue);
    req->buf_len =
        (t_u32) buf_size - sizeof(mlan_ioctl_req) - sizeof(wait_queue);
