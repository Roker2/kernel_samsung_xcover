...
woal_set_bg_scan(moal_private * priv, char *buf, int length)
{
    ...
    scan_cfg.report_condition = BG_SCAN_SSID_MATCH;
    ...
    while (buf_left >= 2) {
        ...
        case WEXT_BGSCAN_RSSI_SECTION:
            scan_cfg.report_condition = BG_SCAN_SSID_RSSI_MATCH;
            scan_cfg.rssi_threshold = ptr[1];
            PRINTM(MIOCTL, "BG scan: rssi_threshold=%d\n",
                   (int) scan_cfg.rssi_threshold);
            ptr += 2;
            buf_left -= 2;
            break;
        case WEXT_BGSCAN_REPEAT_SECTION:
            scan_cfg.repeat_count = (t_u16) ptr[1];
            PRINTM(MIOCTL, "BG scan: repeat_count=%d\n",
                   (int) scan_cfg.repeat_count);
            ptr += 2;
            buf_left -= 2;
            break;
        case WEXT_BGSCAN_INTERVAL_SECTION:
            scan_cfg.scan_interval = (ptr[2] << 8 | ptr[1]) * 1000;
            PRINTM(MIOCTL, "BG scan: scan_interval=%d\n",
                   (int) scan_cfg.scan_interval);
            ptr += 3;
            buf_left -= 3;
            break;
        default:
    ...
    if (num_ssid) {
        ...
        case WIFI_FREQUENCY_BAND_2GHZ:
            scan_cfg.chan_list[0].radio_type = 0 | BAND_SPECIFIED;
            break;
        case WIFI_FREQUENCY_BAND_5GHZ:
            scan_cfg.chan_list[0].radio_type = 1 | BAND_SPECIFIED;
            break;
        default:
            break;
        }
        scan_cfg.bss_type = MLAN_BSS_MODE_INFRA;
        scan_cfg.action = BG_SCAN_ACT_SET;
        scan_cfg.enable = MTRUE;
        ret = woal_request_bgscan(priv, MOAL_IOCTL_WAIT, &scan_cfg);
    }
  done:
    LEAVE();
    return ret;
